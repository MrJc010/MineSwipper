...
;
		...
		while(!queue.isEmpty()){
			...
			if(mineField[element] != MINE && clearedCells[element] != true){
				clearedCells[element] = true;
				if(mineField[element] == 0){
					nb = adjacentCells(element);
				}
			}
			Iterator<Integer> itr = nb.iterator();
			while(itr.hasNext()){
				int num = itr.next();
